...
if (!isnsun && !translucent){
	float occlusion = calcSSDO(fragpos.xyz, normalT.xyz);
	ao = pow(1.0-occlusion, ao_strength);
}
#endif
	
	//Emissive blocks lighting and colors
	float torch_lightmap = 16.0-min(15.0,(texture2D(gdepth,texcoord).z-0.5/16.0)*16.0*16.0/15.0);
...
float diffuse = translucent? 0.75 : clamp(dot(normalT, normalize(shadowLightPosition))*1.02-0.02,0.0,1.0);	
#ifdef Shadows
...
if(diffuse > 0.0 && rainStrength < 0.9){
	...
	float bias = translucent? 0.0010 : distortion*distortion*(0.0050*tan(acos(diffuse)));
